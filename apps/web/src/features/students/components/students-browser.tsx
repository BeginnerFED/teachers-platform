...
export function StudentsBrowser({
  ...
  const filterLabel: Record<(typeof STUDENT_LINKS)[number], string> = {
    ...
    unlinked: t.students.filterUnlinked,
  }
  ...
  return (
    <div className="rounded-md border">
      <div className="flex flex-wrap items-center gap-2 border-b p-3">
        ...
        <Select
          value={query.link ?? ALL}
          onValueChange={(value) => navigate({ link: value === ALL ? null : value })}
        >
          <SelectTrigger className="w-[190px]">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value={ALL}>{t.students.filterAll}</SelectItem>
      ...
      {pending ? <StudentsTableSkeleton rows={Math.min(meta.perPage, 8)} /> : children}
