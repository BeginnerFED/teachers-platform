...
type Tone = AssignmentStatus | 'late'
...
const tone = (status: AssignmentStatus, late: boolean): Tone =>
  late && status === 'assigned' ? 'late' : status
...
export function StatusBadge({
  ...
  return (
    ...
    >
      {key === 'late' ? t.homework.overdue : t.homework.status[status]}
    </Badge>
  )
}
...
export function StatusDot({
  ...
  return (
    <span
      aria-hidden
      className={cn('size-1.5 shrink-0 rounded-full', DOT[tone(status, late)], className)}
    />
  )
}
